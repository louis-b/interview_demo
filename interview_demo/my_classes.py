...
class Widget():
    vendors = ['Alan', 'Bob']
    origins = ['USA', 'France', 'Germany']
    def __init__(self):
        self.vendor = random.choice(Widget.vendors)
        self.origin = random.choice(Widget.origins)
...
class Cart():
    ...
    def __init__(self):
        ...
        for i in range(Cart.n):
            self.w[i] = Widget()
        self.add(0, 0)  # calculate cart and total

    def delete(self, id1):
        self.w[id1].amount = 0
        self.w[id1].subtotal = 0
        self.cart = [k for k in range(Cart.n) if self.w[k].amount > 0]
        self.cart_size = sum([self.w[k].amount for k in range(Cart.n)])
        self.total = sum([self.w[k].subtotal for k in self.cart])

    def add(self, amount, id1):
        # add to the cart
        self.w[id1].amount += amount
        self.w[id1].subtotal += self.w[id1].price * amount
        self.cart = [k for k in range(Cart.n) if self.w[k].amount > 0]
        self.cart_size = sum([self.w[k].amount for k in range(Cart.n)])
        self.total = sum([self.w[k].subtotal for k in self.cart])
